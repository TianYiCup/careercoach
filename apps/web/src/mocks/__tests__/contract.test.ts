...
describe('POST /v1/sessions/:id/turns (SSE)', () => {
  ...
  it('emits the four event types in the expected order', async () => {
    ...
    // PR-OPT1: arc/mood now land AFTER opponent.done (the arc/mood engine
    // runs concurrently with the reply stream), arc before mood.
    ...
    expect(doneIdx).toBeLessThan(arcIdx)
