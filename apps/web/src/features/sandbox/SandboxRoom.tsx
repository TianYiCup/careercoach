 ...
 * Custom scenario: 未开始时显示场景选择 + 自定义输入
 */
...
export function SandboxRoom({
  ...
  /** Start a preset scenario */
  const handleStartPreset = (scenarioId: string, personaId: string, goal: string) => {
    startSession({ mode: 'sandbox', scenario_id: scenarioId, persona_id: personaId, user_goal: goal })
  }

  /** Generate custom scenario then start session */
  const handleStartCustom = async () => {
    const res = await generateCustom(customDesc)
    if (res) {
      startSession({ mode: 'sandbox', scenario_id: res.scenario_id, persona_id: res.persona_title, user_goal: customDesc.trim() })
    }
  }
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden">
      ...
      {!state.started && !state.score && (
        <div className="relative z-10 flex-1 flex flex-col items-center px-4 py-8 overflow-y-auto">
          <div className="w-full max-w-md space-y-6">
            <div className="flex items-center justify-between">
              <button type="button" onClick={onExit} className="text-ink-text-2 text-sm hover:text-ink-text transition-colors">&larr; 返回</button>
              <MascotReaction expression="confident" size="sm" />
            </div>
            <h2 className="text-2xl font-display text-ink-text text-center">选择对练场景</h2>
            <GlassCard className="space-y-3">
              <p className="text-xs text-ink-text-3 font-body">经典场景</p>
              <div className="space-y-2">
                {([
                  { id: 'scenario_campus_overtime', persona: 'boss_strict', goal: '拒绝加班且不撕破脸', label: '周末加班谈判' },
                  { id: 'scenario_intern_salary', persona: 'hr_friendly', goal: '薪资谈判不卑不亢', label: '实习转正谈薪资' },
                  { id: 'scenario_roommate_noise', persona: 'roommate_lazy', goal: '让室友安静但不撕逼', label: '室友深夜打游戏' },
                ] as const).map(s => (
                  <button key={s.id} type="button" onClick={() => handleStartPreset(s.id, s.persona, s.goal)} className="w-full px-4 py-3 rounded-radius-md bg-ink-card border border-ink-line text-left hover:border-vivid-purple transition-colors">
                    <span className="text-sm text-ink-text font-body font-medium">{s.label}</span>
                  </button>
                ))}
              </div>
            </GlassCard>
            <GlassCard className="space-y-3">
              <p className="text-xs text-ink-text-3 font-body">自定义场景</p>
              <textarea value={customDesc} onChange={e => setCustomDesc(e.target.value)} placeholder="描述你的场景，比如「导师让我帮忙做私活该怎么拒绝」...（至少 30 字）" rows={3} className="w-full rounded-radius-md bg-ink-card px-4 py-3 text-sm text-ink-text font-body border border-ink-line focus:border-vivid-purple focus:outline-none transition-colors resize-none" />
              {customState.error && <p className="text-xs text-vivid-orange font-body">{customState.error}</p>}
              <button type="button" onClick={handleStartCustom} disabled={customState.isGenerating || customDesc.trim().length < 30} className="px-6 py-2.5 rounded-radius-pill gradient-vivid text-white text-sm font-body font-medium hover:scale-105 transition-transform disabled:opacity-50 disabled:hover:scale-100">
      ...
      {state.started && (
        <>
          <header className="relative z-10 flex items-center justify-between px-4 py-3 glass">
            <button type="button" onClick={handleExitClick} className="text-ink-text-2 text-sm hover:text-ink-text transition-colors">← 退出</button>
            <div className="flex items-center gap-2">
              <MascotReaction expression={state.mascotExpression} size="sm" />
              <div className="flex flex-col">
                <span className="text-sm font-body text-ink-text leading-tight">对练中</span>
                <span className="text-xs text-ink-text-3 leading-tight">{state.turnsUsed}/{totalTurns} 回合</span>
              </div>
            </div>
      ...
      {/* Moderation redirect — crisis hotline */}
